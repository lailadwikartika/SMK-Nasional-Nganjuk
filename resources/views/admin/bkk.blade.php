        ...
        <section class="section">
            <div class="row">
                ...
                <div class="col-lg-12 mb-3">
                    <a href="{{ route('bkk.create') }}" class="btn btn-primary">
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Data Berita BKK</h5>
                            ...
                            <div class="table-responsive">
                                <table class="table datatable">
                                    ...
                                    <tbody>
                                        @forelse($data as $bkk)
                                            <tr>
                                                <td>{{ $bkk->judul_lowongan }}</td>
                                                <td>{{ $bkk->nama_perusahaan }}</td>
                                                <td>{{ Str::limit($bkk->persyaratan, 50) }}</td>
                                                <td>{{ \Carbon\Carbon::parse($bkk->tanggal_dibuka)->translatedFormat('d M Y') }}</td>
                                                <td>{{ \Carbon\Carbon::parse($bkk->tanggal_ditutup)->translatedFormat('d M Y') }}</td>
                                                <td>
                                                    @if ($bkk->foto)
                                                        <img src="{{ asset('storage/' . $bkk->foto) }}" alt="Foto" width="60" class="rounded">
                                                    @else
                                                        <span class="text-muted">Tidak ada foto</span> <!-- UBAH: dari "-" ke "Tidak ada foto" -->
                                                    @endif
                                                </td>
                                                
                                                <td>
                                                    <a href="{{ route('bkk.edit', $bkk->id_bkk) }}"
                                                        class="btn btn-sm btn-warning me-1 mb-1"><i class="bi bi-pencil-square"></i> Edit</a>
                                                    <form id="delete-form-{{ $bkk->id_bkk }}" action="{{ route('bkk.destroy', $bkk->id_bkk) }}" method="POST" class="d-inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" class="btn btn-sm btn-danger" onclick="confirmDelete({id: '{{ $bkk->id_bkk }}', title: 'Hapus Lowongan Pekerjaan?', name: '{{ $bkk->judul_lowongan }}', subtext: '{{ $bkk->perusahaan }}', type: 'lowongan'})">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="7" class="text-center">Belum ada data BKK.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
